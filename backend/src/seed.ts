...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.property.createMany({ data: [
    ...
    for (const professional of professionals) {
      const user = await tx.user.create({ data: { email: professional.email, password: passwordHash, role: 'PROFESSIONAL' } });
      await tx.profile.create({ data: { name: professional.name, userId: user.id, roleTitle: professional.roleTitle, location: professional.location, hourlyRate: professional.hourlyRate, rating: professional.rating, reviewsCount: professional.reviewsCount, verified: professional.verified, tags: professional.tags, avatarUrl: professional.avatarUrl } });
    }
  });
